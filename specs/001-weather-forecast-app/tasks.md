...
**Input**: Design documents from `/specs/feature/impl-001-weather-forecast-app/`
...
```mermaid
gantt
    title 実装スケジュール（相対日付方式・開始日は任意に変更可能）
...
- **Phase 1 (Setup)**: 3時間
- **Phase 2 (Foundational)**: 10時間
- **Phase 3 (US1-MVP)**: 27時間
- **Phase 4 (US2)**: 10時間
- **Phase 5 (US3)**: 9時間
- **Phase 6 (Polish)**: 16時間
- **合計**: 75時間
...
- 土日を自動除外（excludes weekends）
- 年末年始の休日は開始日を調整して除外
- 相対日付方式（after）により、開始日（T001の日付）のみ変更すれば全体のスケジュールが自動調整される
